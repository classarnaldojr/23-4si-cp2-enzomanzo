...
#sorted = organize contour list
#reverse True = organize + to -
...
def draw_contour(rgb, contours):
    ...
    return cv2.drawContours(contours_img, [contours[0], contours[1]], -1, (0, 0, 0), 0)
...
def object_center(contour):
    return int(cv2.moments(contour)['m10'] / cv2.moments(contour)['m00']) + int(
...
def ident_winner(move1, move2):
    win_dict = {
        'Pedra': {'Tesoura': 'Jogador 1 ganhou!', 'Papel': 'Jogador 2 ganhou!'},
        'Papel': {'Pedra': 'Jogador 1 ganhou!', 'Tesoura': 'Jogador 2 ganhou!'},
        'Tesoura': {'Papel': 'Jogador 1 ganhou!', 'Pedra': 'Jogador 2 ganhou!'}
    }
    if (move1 == move2):
        return 'Empate'
    elif move1 in win_dict.keys() and move2 in win_dict[move1].keys():
        return win_dict[move1][move2]
    else:
...
while rval:
    ...
    cv2.putText(draw_contours, 'Player 1 jogou ' + move1 + ' e ' + 'Player 2 jogou ' + move2, (600, 150),
